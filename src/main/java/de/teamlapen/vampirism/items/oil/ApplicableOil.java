...
public abstract class ApplicableOil extends Oil implements IApplicableOil {
    ...
    public Optional<ITextComponent> getToolTipLine(ItemStack stack, IApplicableOil oil, int duration) {
        ...
        return Optional.of(new TranslationTextComponent(String.format("oil.%s.%s",this.getRegistryName().getNamespace(), this.getRegistryName().getPath())).withStyle(TextFormatting.LIGHT_PURPLE).append(" ").append(new TranslationTextComponent("Status").withStyle(status)));
    }
}
